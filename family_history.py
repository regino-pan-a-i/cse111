...
def print_death_age(people_dict):
    ...
    print()
    print('Ages at Death')
    for person_key, values in people_dict.items():
...
def print_marriages(marriages_dict, people_dict):
    ...
    """
    print('Marriges')
    for marriage_key, values in marriages_dict.items():
        h_key = values[HUSBAND_KEY_INDEX]
        year = values[WEDDING_YEAR_INDEX]
        w_key = values[WIFE_KEY_INDEX]

        h_list = people_dict[h_key]
        h_name = h_list[NAME_INDEX]
        h_birth= h_list[BIRTH_YEAR_INDEX]
        h_age_at_wedding = year - h_birth 

        w_list = people_dict[w_key]
        w_name = w_list[NAME_INDEX]
        w_birth= w_list[BIRTH_YEAR_INDEX]
        w_age_at_wedding = year - w_birth 


        print(f'{h_name} {h_age_at_wedding} > {w_name} {w_age_at_wedding}')
